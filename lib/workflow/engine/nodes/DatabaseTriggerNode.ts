...
export class DatabaseTriggerNode implements NodeClass {
  type = 'DatabaseTriggerNode';
  name = 'Database Trigger';
  description = 'Database change triggers';
  ...
  async execute(context: ExecutionContext, config: Record<string, any>): Promise<NodeExecutionResult> {
    ...
    try {
      ...
      const result = await this.triggerDatabase({ table: table || '' });
  ...
  private async triggerDatabase(triggerData: { table: string }): Promise<any> {
