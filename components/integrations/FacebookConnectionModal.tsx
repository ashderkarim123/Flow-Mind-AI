...
export const FacebookConnectionModal = ({ open, onClose, onConnectionSuccess }: Props) => {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/integrations/facebook/auth', {
        method: 'POST',
        headers: {
      ...
      if (!data.success) {
        setError(data.error || 'Failed to initiate Facebook auth');
        return;
      }
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-md bg-neutral-900 text-white border border-white/10">
        <DialogHeader>
          ...
          <DialogDescription>
            Authorize access to your Facebook Page for messaging and insights.
          </DialogDescription>
        </DialogHeader>
        ...
        <Card className="bg-white/5 border border-white/10 text-white">
          ...
          <CardContent className="space-y-2 text-sm text-white/80">
            <div>• Click Connect and login with Facebook</div>
            <div>• Select the Page you want to use</div>
