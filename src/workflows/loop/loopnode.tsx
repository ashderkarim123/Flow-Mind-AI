    ...
    >
      <div className="flex items-center gap-3">
        <div className={`p-2 rounded-lg ${isConfigured ? 'bg-yellow-100' : 'bg-gray-100'}`}>
          <RotateCw className={`w-5 h-5 ${isConfigured ? 'text-yellow-600' : 'text-gray-400'}`} />
        </div>
        <div className="flex-1">
          <h3 className="font-semibold text-gray-900 text-sm">{data?.label || 'Loop'}</h3>
          <p className="text-xs text-gray-500">Repeat for each</p>
        </div>
        {isConfigured && (
