...
export function OpenAINode({ data, selected }: OpenAINodeProps) {
    ...
    >
      <div className="flex items-center gap-3">
        <div className={`p-2 rounded-lg ${isConfigured ? 'bg-violet-100' : 'bg-gray-100'}`}>
          <Brain className={`w-5 h-5 ${isConfigured ? 'text-violet-600' : 'text-gray-400'}`} />
        </div>
        <div className="flex-1">
          <h3 className="font-semibold text-gray-900 text-sm">{data?.label || 'OpenAI'}</h3>
          <p className="text-xs text-gray-500">Use OpenAI API</p>
        </div>
        {isConfigured && (
